...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['ajax'])) {
    ...
    if ($_POST['action'] == 'add') {
        ...
        $stmt = $conn->prepare("INSERT INTO Equipment (EquipmentName, PurchaseDate, `Condition`, Quantity) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("sssi", $name, $purchase_date, $condition, $quantity);
    ...
    if ($_POST['action'] == 'edit') {
        ...
        $stmt = $conn->prepare("UPDATE Equipment SET EquipmentName=?, PurchaseDate=?, `Condition`=?, Quantity=? WHERE EquipmentID=?");
        $stmt->bind_param("sssii", $name, $purchase_date, $condition, $quantity, $id);
...
if (isset($_GET['get_equipment'])) {
    header('Content-Type: application/json');
    $id = intval($_GET['get_equipment']);
...
if (isset($_GET['delete'])) {
    ...
    $stmt->bind_param("i", $id);
    $stmt->execute();
    header("Location: equipment_manage.php?msg=deleted");
    $stmt->close(); exit;
}
...
$equipment = $conn->query("SELECT * FROM Equipment ORDER BY EquipmentName ASC");
?>
...
<div class="content-wrapper">
    <section class="content-header">
        <h1>Equipment <small>Manage gym equipment</small></h1>
        <ol class="breadcrumb">
            ...
            <div class="col-xs-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">All Equipment</h3>
                        <div class="box-tools">
                    ...
                    <div class="box-body">
                        <?php if (isset($_GET['msg'])): ?>
                            <div class="alert alert-success alert-dismissible">
                                ...
                                <i class="fa fa-check"></i> Equipment deleted successfully!
                            </div>
                        <?php endif; ?>
                            ...
                            <table id="dataTable" class="table table-hover table-bordered">
                                ...
                                <tbody>
                                    <?php while ($row = $equipment->fetch_assoc()): 
                                        $conditionClass = 'label-success';
                                        if ($row['Condition'] == 'Good') $conditionClass = 'label-info';
                                        elseif ($row['Condition'] == 'Fair') $conditionClass = 'label-warning';
                                        elseif ($row['Condition'] == 'Poor') $conditionClass = 'label-danger';
                                    ?>
                                    <tr>
                                        <td><?= $row['EquipmentID'] ?></td>
                                        <td><strong><?= htmlspecialchars($row['EquipmentName']) ?></strong></td>
                                        <td><?= $row['PurchaseDate'] ? date('d M Y', strtotime($row['PurchaseDate'])) : 'N/A' ?></td>
                                        <td><span class="label <?= $conditionClass ?>"><?= htmlspecialchars($row['Condition']) ?></span></td>
                                        <td><?= $row['Quantity'] ?></td>
                                        <td class="text-nowrap">
                                            <button onclick="openEditModal(<?= $row['EquipmentID'] ?>)" class="btn btn-warning btn-xs">
                                                <i class="fa fa-edit"></i> Edit
                                            </button>
                                            <a href="equipment_manage.php?delete=<?= $row['EquipmentID'] ?>" onclick="return confirm('Are you sure?')" class="btn btn-danger btn-xs">
    ...
    <div class="modal-container">
        ...
        <form id="addForm" onsubmit="return false;">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-modal btn-modal-success">Add Equipment</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="modal-overlay">
    <div class="modal-container">
        ...
        <form id="editForm" onsubmit="return false;">
            <input type="hidden" id="edit_id" name="equipment_id">
            <div class="modal-body">
                <div id="editError" class="alert-modal alert-danger" style="display:none;"></div>
                <div class="form-group">
                    <label>Equipment Name *</label>
                    <input type="text" id="edit_name" name="equipment_name" required>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label>Purchase Date</label>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-modal btn-modal-primary">Update Equipment</button>
            </div>
        </form>
    </div>
</div>
...
function openEditModal(id) {
    fetch('equipment_manage.php?get_equipment=' + id)
        .then(r => r.json())
        .then(d => {
            document.getElementById('edit_id').value = d.EquipmentID;
            document.getElementById('edit_name').value = d.EquipmentName;
            document.getElementById('edit_purchase_date').value = d.PurchaseDate || '';
            document.getElementById('edit_condition').value = d.Condition || 'Good';
            document.getElementById('edit_quantity').value = d.Quantity || 1;
...
document.getElementById('addForm').addEventListener('submit', function(e) {
    ...
    fetch('equipment_manage.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => {
            if (d.success) { window.location.href = 'equipment_manage.php'; }
