...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['ajax'])) {
    ...
    if ($_POST['action'] == 'add') {
        ...
        if (empty($member_id) || empty($amount)) {
            $response['message'] = 'Member and Amount are required';
    ...
    if ($_POST['action'] == 'edit') {
        ...
        $stmt = $conn->prepare("UPDATE Payment SET MemberID=?, Amount=?, Date=?, PaymentMethod=? WHERE PaymentID=?");
        $stmt->bind_param("idssi", $member_id, $amount, $date, $method, $id);
...
$payments = $conn->query("SELECT p.*, m.Name as MemberName FROM Payment p 
    LEFT JOIN Member m ON p.MemberID = m.MemberID 
    ORDER BY p.Date DESC");
$members = $conn->query("SELECT * FROM Member ORDER BY Name");

// Calculate totals
$total_revenue = $conn->query("SELECT SUM(Amount) as total FROM Payment")->fetch_assoc()['total'] ?? 0;
...
<div class="content-wrapper">
    <section class="content-header">
        <h1>Payments <small>Manage member payments</small></h1>
        <ol class="breadcrumb">
            ...
            <div class="col-md-6">
                <div class="info-box bg-green">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">Rs. <?= number_format($total_revenue) ?></span>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                    ...
                    <div class="box-body">
                        <?php if (isset($_GET['msg'])): ?>
                            <div class="alert alert-success alert-dismissible">
                                ...
                                <i class="fa fa-check"></i> Payment deleted successfully!
                            </div>
                        <?php endif; ?>
                            ...
                            <table id="dataTable" class="table table-hover table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="label label-success"><?= htmlspecialchars($row['PaymentMethod']) ?></span></td>
                                        <td class="text-nowrap">
                                            <button onclick="openEditModal(<?= $row['PaymentID'] ?>)" class="btn btn-warning btn-xs">
                                                <i class="fa fa-edit"></i> Edit
                                            </button>
                                            <a href="payments_manage.php?delete=<?= $row['PaymentID'] ?>" onclick="return confirm('Are you sure?')" class="btn btn-danger btn-xs">
    ...
    <div class="modal-container">
        ...
        <form id="addForm" onsubmit="return false;">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-modal btn-modal-success">Save Payment</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="modal-overlay">
    <div class="modal-container">
        ...
        <form id="editForm" onsubmit="return false;">
            <input type="hidden" id="edit_id" name="payment_id">
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label>Payment Method</label>
                    <select id="edit_method" name="payment_method">
                        <option value="Cash">Cash</option>
                        <option value="Credit Card">Credit Card</option>
                        <option value="Debit Card">Debit Card</option>
                        <option value="Bank Transfer">Bank Transfer</option>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-modal btn-modal-primary">Update Payment</button>
            </div>
        </form>
    </div>
</div>
...
function openEditModal(id) {
    fetch('payments_manage.php?get_payment=' + id)
        .then(r => r.json())
        .then(d => {
            document.getElementById('edit_id').value = d.PaymentID;
            ...
            document.getElementById('edit_method').value = d.PaymentMethod;
...
document.getElementById('addForm').addEventListener('submit', function(e) {
    ...
    fetch('payments_manage.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(d => {
            if (d.success) { window.location.href = 'payments_manage.php'; }
